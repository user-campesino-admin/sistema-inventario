...
# Corrección crítica para SQLAlchemy 2.0 con Supabase
if DATABASE_URL and DATABASE_URL.startswith("postgres://"):
    DATABASE_URL = DATABASE_URL.replace("postgres://", "postgresql://", 1)

# Asegurar parámetros de seguridad SSL requeridos por Supabase
...
# Intentar crear las tablas de forma segura sin romper la app si hay latencia de red
try:
    Base.metadata.create_all(bind=engine)
except Exception as db_error:
    st.warning(advertencia := f"Nota de conexión a base de datos: {db_error}")
