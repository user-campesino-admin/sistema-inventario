...
# --- 1. CARGA DE TXT (OPTIMIZADA PARA ARCHIVOS PESADOS) ---
if menu == "Cargar TXT":
    ...
    if uploaded_files and ciudad and st.button("Procesar", key="btn_procesar_txt"):
        ...
        with st.spinner("Procesando y guardando archivo(s) por lotes para evitar sobrecarga..."):
            with SessionLocal() as session:
                try:
                    total_registros = 0
                    for file in uploaded_files:
                        # Leer el archivo grande en bloques de 10,000 líneas para proteger la memoria RAM
                        chunk_size = 10000
                        for chunk in pd.read_csv(file, header=None, names=columnas, sep='\t', dtype=str, encoding='latin-1', low_memory=False, chunksize=chunk_size):
                            
                            chunk['serie'] = chunk['serie'].astype(str).str.strip()
                            chunk['ciudad_origen'] = ciudad
                            chunk['nombre_archivo'] = file.name 
                            
                            session.bulk_insert_mappings(RegistroInventario, chunk.to_dict(orient='records'))
                            session.commit() # Guardar lote por lote de manera segura
                            total_registros += len(chunk)
                    
                    st.success(f"¡Carga exitosa! Se guardaron {total_registros:,} registros en total de forma segura.")
...
# --- 3. GESTIÓN DE HISTORIAL ---
    ...
    with SessionLocal() as session:
        ...
        if total_registros > 0:
            st.subheader("📊 Vista previa (Mostrando los últimos 1,000 registros para proteger la memoria)")
            # Consulta limitada para proteger el límite de tamaño del navegador (MessageSizeError)
            query_limitada = session.query(RegistroInventario).order_by(RegistroInventario.id.desc()).limit(1000)
            df_historico = pd.read_sql(query_limitada.statement, session.bind)
